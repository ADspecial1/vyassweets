...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="product-card-hover bg-white rounded-2xl overflow-hidden border border-orange-100/60 group flex flex-col shadow-sm">
      {/* Image / placeholder */}
      <Link to={`/product/${product.slug}`} className="relative overflow-hidden block">
        {product.images[0] ? (
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #FFF3E0, #FFECD2)' }}
          >
            <div
            ...
            <span className="text-xs text-[#7A5C5A] font-semibold">{product.weight}{product.unit}</span>
          </div>
        )}
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5">
          {pct >= 5 && (
            <span className="bg-[#A8D5BA] text-[#4A2C2A] text-[10px] font-black px-2 py-0.5 rounded-full shadow-sm">
              {pct}% OFF
            </span>
          )}
          {product.featured && (
        ...
        {/* Hover overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#4A2C2A]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      </Link>

      {/* Info */}
      <div className="p-3.5 flex flex-col flex-1">
        <Link to={`/product/${product.slug}`} className="flex-1 mb-3">
          <h3 className="font-bold text-[#4A2C2A] text-sm leading-snug line-clamp-2 group-hover:text-[#E8762A] transition-colors">
            {product.name}
          </h3>
          <p className="text-xs text-[#7A5C5A] mt-1 font-medium">{product.weight}{product.unit}</p>
        </Link>

        <div className="flex items-center justify-between">
          <div>
            <span className="font-black text-[#4A2C2A] text-base">{formatINR(product.price)}</span>
            {product.mrp > product.price && (
              <span className="text-xs text-[#7A5C5A] line-through ml-1.5">{formatINR(product.mrp)}</span>
            )}
          </div>
          ...
          <button
            ...
            className={`btn-shine flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed ${
              inCart
                ? 'bg-[#A8D5BA]/20 text-[#4A2C2A] border border-[#A8D5BA] hover:bg-[#A8D5BA]/30'
                : 'text-white hover:shadow-md hover:-translate-y-0.5'
            }`}
            style={!inCart ? { background: 'linear-gradient(135deg, #F4A261, #E8762A)' } : undefined}
          >
            {inCart
              ? <><Check size={13} /> Added</>
